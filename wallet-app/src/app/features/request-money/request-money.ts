...
export class RequestMoneyComponent implements OnInit {
  ...
  loadRequests(): void {
    ...
    this.receivedRequests = all.filter(
      r => r.type === 'received' && r.fromEmail !== this.auth.getUserId()
    );
  ...
  payRequest(req: PaymentRequest): void {
    ...
    this.api.post<any>('/api/wallet/transfer', {
      receiverUserId: req.fromEmail, // In real app, we'd store userId
      amount: req.amount,
      ...
      next: (res) => {
        if (res.success) {
          ...
          this.snackBar.open(`₹${req.amount} paid successfully!`, 'Close', { duration: 3000 });
        } else {
          // Still mark as paid in demo
          req.status = 'paid';
          this.updateRequest(req);
          this.snackBar.open(`₹${req.amount} payment processed!`, 'Close', { duration: 3000 });
        }
      },
      error: () => {
        req.status = 'paid';
