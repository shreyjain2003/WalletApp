        ...
        <div class="user-card" *ngFor="let user of filteredUsers">
          ...
          <div class="wallet-section" *ngIf="user.wallet">
            <div class="wallet-balance">
              ...
              <span class="balance-val">₹{{ user.wallet.balance | number:'1.2-2' }}</span>
...
export class UserListComponent implements OnInit {
  ...
  async loadWallets(): Promise<void> {
    for (const user of this.users) {
      try {
        const res: any = await this.api.get<any>(
          `/api/wallet/by-email?email=${user.email}`).toPromise();
        if (res?.success) {
  ...
  saveWalletChanges(): void {
    ...
    this.api.put<any>('/api/wallet/admin/adjust', {
      ...
      next: (res) => {
        if (res.success) {
          // Also update lock status
          this.api.put<any>('/api/wallet/admin/lock', {
            userId: this.editingUser.userId,
            isLocked: this.editingLocked
          }).subscribe({
            next: () => {
              this.snackBar.open('Wallet updated successfully!', 'Close', { duration: 3000 });
            ...
            error: () => {
              this.snackBar.open('Balance updated but lock failed.', 'Close', { duration: 3000 });
              this.saving = false;
              this.editingUser = null;
            }
          });
