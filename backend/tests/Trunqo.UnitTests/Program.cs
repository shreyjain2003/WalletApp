...
static void WalletGetOrCreateCreatesWallet()
{
    ...
    var service = new WalletService.Services.WalletService(repo, new FakeWalletPublisher(), BuildConfig(), null!);
...
file sealed class FakeAuthPublisher : AuthService.Services.IRabbitMqPublisher
{
    ...
    public Task PublishAsync<T>(string queueName, T message) { Published.Add((queueName, message!)); return Task.CompletedTask; }
}
...
file sealed class FakeAuthRepository : IAuthRepository
{
    ...
    public Task AddPasswordResetSessionAsync(PasswordResetSession session) => Task.CompletedTask;
    public Task<PasswordResetSession?> GetLatestPasswordResetSessionAsync(Guid userId, string otpHash) => Task.FromResult<PasswordResetSession?>(null);
    public Task<PasswordResetSession?> GetLatestVerifiedPasswordResetSessionAsync(Guid userId, string tempTokenHash) => Task.FromResult<PasswordResetSession?>(null);
    public Task InvalidatePasswordResetSessionsAsync(Guid userId, string reason, Guid? excludeSessionId = null) => Task.CompletedTask;
}
